...
#Location of the most recent open database available at the government website (September 21st in our case) for the per-occurrence date analysis
covid_mx <- read_csv("~/Protocolos de investigación/BASE COVID GOB/Bases de datos diarias/200921COVID19MEXICO.csv")
...
#load("~/Protocolos de investigación/BASE COVID GOB/covid_mex/base_completa_per_reporting_date_sep_21.Rda")
#load("~/Protocolos de investigación/BASE COVID GOB/covid_mex/uci_def_per_reporting_date_aug_31.Rda")
...
bases_csv <- c(str_c("~/Protocolos de investigación/BASE COVID GOB/Bases de datos diarias/20",rep("04",19), seq(12,30,1) ,"COVID19MEXICO.csv"),
                   str_c("~/Protocolos de investigación/BASE COVID GOB/Bases de datos diarias/20",rep("05",31), num_31 ,"COVID19MEXICO.csv"),
                   str_c("~/Protocolos de investigación/BASE COVID GOB/Bases de datos diarias/20",rep("06",30), num_30 ,"COVID19MEXICO.csv"),
                   str_c("~/Protocolos de investigación/BASE COVID GOB/Bases de datos diarias/20",rep("07",31), num_31 ,"COVID19MEXICO.csv"),
                   str_c("~/Protocolos de investigación/BASE COVID GOB/Bases de datos diarias/20",rep("08",31), num_31 ,"COVID19MEXICO.csv"),
                   str_c("~/Protocolos de investigación/BASE COVID GOB/Bases de datos diarias/20",rep("09",21), num_21 ,"COVID19MEXICO.csv")
               )
...
nombres_bases <- c(str_c("covid_abril_", 12:30),
                   ...
                   str_c("covid_septiembre_", num_21))
...
nombres_bases_pendientes <- c(str_c("covid_abril_", 12:30, "_pendiente"),
                   ...
                   str_c("covid_septiembre_", num_21,"_pendiente"))
...
nombres_bases_resultado <- c(str_c("covid_abril_", 12:30, "_resultado"),
                   ...
                   str_c("covid_septiembre_", num_21,"_resultado"))
...
```{r Per-reporting date country-level case estimation, include = F, eval = T, message = F, warning=F}
...
for(i in 2:length(nombres_bases)){
  base_completa <- rbind(base_completa,read_csv(bases_csv[i]) %>% 
                           ...
                           filter(RESULTADO !=3  & FECHA_INGRESO >= "2020-02-27" & FECHA_INGRESO <= "2020-09-21" & 
                                    FECHA_INGRESO <= FECHA_ACTUALIZACION) %>% 
                           mutate(FECHA_CONTAGIO_5 = FECHA_SINTOMAS - 5,
                                  DIAS_C_INF_INGRESO = as.double(FECHA_INGRESO - FECHA_CONTAGIO_5),
                                  RESULTADO = ifelse(RESULTADO == 1, T, F),
...
#save(base_completa_per_reporting_date, file = "base_completa_per_reporting_date_sep_21.Rda")
#load("~/Protocolos de investigación/BASE COVID GOB/covid_mex/base_completa_per_reporting_date_sep_21.Rda")
...
```{r Per-reporting date country-wide ICU admission and deaths estimation, echo=F, eval = T, message = F, warning=F}
base_uci_defunciones_diario <- read_csv(bases_csv[1]) %>% 
  mutate(FECHA_CONTAGIO_5 = FECHA_SINTOMAS - 5,
         DIAS_C_INF_INGRESO = as.double(FECHA_INGRESO - FECHA_CONTAGIO_5)) %>% 
  filter(RESULTADO !=3  & FECHA_INGRESO >= "2020-02-27" & FECHA_INGRESO <= "2020-09-21" & DIAS_C_INF_INGRESO <=25) %>% 
...
for(i in seq_along(nombres_bases)){ 
  base_uci_defunciones_diario <- rbind(base_uci_defunciones_diario,read_csv(bases_csv[i]) %>% 
  mutate(FECHA_CONTAGIO_5 = FECHA_SINTOMAS - 5,
         DIAS_C_INF_INGRESO = as.double(FECHA_INGRESO - FECHA_CONTAGIO_5)) %>% 
  filter(RESULTADO !=3  & FECHA_INGRESO >= "2020-02-27" & FECHA_INGRESO <= "2020-09-21" & DIAS_C_INF_INGRESO <=25) %>% 
  select(FECHA_ACTUALIZACION, ID_REGISTRO, RESULTADO, FECHA_DEF,
...
#save(uci_def_per_reporting_date, file = "uci_def_per_reporting_date_sep_21.Rda")
```
...
fechas_tabla_1 <- data.frame(fechas_nombre = c(str_c(lubridate::month(seq.Date(from=as.Date("2020-04-13"), 
                                     to =as.Date("2020-09-21"),by = 7),label = TRUE, abbr = F),
                      " ",
                      lubridate::day(seq.Date(from=as.Date("2020-04-13"), 
                                     to =as.Date("2020-09-21"),by = 7))),"September 21"),
                      fechas = c(seq.Date(from=as.Date("2020-04-13"), 
                                     to =as.Date("2020-09-21"), by = 7),ymd("2020-09-21")))
...
casos_diarios_acumulados_c_fn <- ggplot(filter(base_completa_per_reporting_date,fecha_actualizacion >="2020-04-13"),
  ...
  scale_fill_manual(breaks = c(str_c("True negative (total = ", last(base_completa_per_reporting_date$pruebas_neg_med_acum),")"),
                               ...
                               str_c("Official positive (total = ", 
                                     last(base_completa_per_reporting_date$pruebas_pos_oficial_acum), ")")),
                    values=c("deepskyblue4", "gray50", "darkorange2"))+
  scale_y_continuous(breaks = seq(0, 1600000, 100000))+
  scale_x_date(date_breaks = "1 week",
               date_labels ="%B %d",
               expand = c(0.03, 0.03))+
  theme_minimal()+
...
casos_diarios_reportados_c_fn+casos_diarios_acumulados_c_fn+
  plot_annotation(tag_levels = "A",
                  title = "Figure 1. Estimated proportion of tested individuals with a false negative result during the study period",
                  caption = "'A' represents new daily test results, 'B' represents accumulated test results. Study period from February 27th - September 21st, shown from April 13th.")&
...
fecha_actualizacion <- seq.Date(from = ymd("2020-04-13"), to = ymd("2020-09-21"), by = 1)
```
...
save(base_preliminar_estados, file = "base_preliminar_estados_sep_21.Rda")
```
...
### Filling dates with no testing reported in certain states (frequent during early epidemic), per reporting date analysis
 base_preliminar_estados_mod <- base_preliminar_estados %>% 
  ...
  full_join(data.frame(FECHA_ACTUALIZACION = seq.Date(from = ymd("2020-04-13"), 
                                                      to = ymd("2020-09-21"), by = 1)), by = "FECHA_ACTUALIZACION") %>%     
...
(ggplot(filter(base_preliminar_estados_mod, ENTIDAD_UM == estados$num_edo[i]),
  ...
  theme(legend.position = c(0.35, 0.7),
        ...
        plot.title = element_text(size = 14, face = "bold")))+
  plot_annotation(
    title = str_c("Figure SE. ", estados$nombre_edo[i]),
    caption = "Study period February 27 - September 21 (shown from April 13). A) represents daily counts, while B) represents accumulated cases.",
...
save_as_docx(flextable(x, cwidth = 1.5) %>% 
               add_header_lines(str_c("Table SE. ", 
...
#Date in which estimated cases first surpassed 50,000
base_completa_per_reporting_date$fecha_actualizacion[base_completa_per_reporting_date$pruebas_pos_med_acum >=50000][1];
base_completa_per_reporting_date$fecha_actualizacion[base_completa_per_reporting_date$pruebas_pos_ub_acum >=50000][1]; 
base_completa_per_reporting_date$fecha_actualizacion[base_completa_per_reporting_date$pruebas_pos_lb_acum >=50000][1]; 
base_completa_per_reporting_date$fecha_actualizacion[base_completa_per_reporting_date$pruebas_pos_oficial_acum >=50000][1]#########
...
nrow(filter(covid_mx, fecha_ingreso >="2020-02-27"&fecha_ingreso <= "2020-09-21"))
...
base_trabajo <- as.data.table(filter(covid_mx, resultado != 3) %>% 
  ...
  filter(dias_c_inf_ingreso <=21 & fecha_ingreso >= "2020-02-27"& fecha_ingreso <= "2020-09-21")
  

fechas_media <- seq.Date(from=ymd("2020-02-27"), to = ymd("2020-09-21"), by = 1)
...
mm$fecha <-  seq.Date(from=ymd("2020-03-04"), to = ymd("2020-09-21"), by = 1)
mm <- right_join(mm, data.frame(fecha = seq.Date(from=ymd("2020-02-27"), to = ymd("2020-09-21"), by = 1)), by = "fecha")

base_trabajo_media <- base_trabajo %>%
  group_by(fecha_ingreso) %>% 
  summarise(media = mean(dias_c_sts)) %>% 
  rename(fecha = fecha_ingreso) %>% 
  full_join(data.frame(fecha=seq.Date(from=ymd("2020-08-06"), to = ymd("2020-09-21"), by = 1)), by = "fecha")
